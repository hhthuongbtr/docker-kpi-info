...
class Command(BaseCommand):
    ...
    def write_log_to_db(self, log_path, server_index):
        ...
        for row in log:
            revenue.bill_id = row["BillID"]
            revenue.server_index = server_index
            revenue.player_id = row["PlayerID"]
            revenue.player_name  = row["PlayerName"].encode('utf-8')
            print(revenue.player_name)
            revenue.pay_money = row["pay_money"]
            revenue.order_time = datetime.datetime.strptime(row["order_time"], '%Y-%m-%d %H:%M:%S')
            revenue.save()